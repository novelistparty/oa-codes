...
% figure
for jj = 1 : (length(r) - 1)
    step = exp(0.25 * 1i * k0 .* (nsqr(:,jj) - 1) * dr) .* psi(:,jj);
    ...
    psi(:,jj+1) =  exp(0.25 * 1i * k0 .* (nsqr(:,jj) - 1) * dr) .* (stepr + 1i * stepi);
...
% %% Plot TL
% figure; 
% pcolor(R, Z, -20 * log10( abs(psi) ./ sqrt(R)))
% shading interp
% grid off
% colorbar
% axis equal
...
% figure; contourf(R, Z, - 20 *log10( abs(psi) ./ sqrt(R) ), v)
% colorbar
% xlim([0 500])
% ylim([0 500])
% colormap 'pink'
